...
cas_propertyAttributes *cas_copyPropertyAttributes (objc_property_t property) {
    ...
    cas_propertyAttributes *attributes = calloc(1, sizeof(cas_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate cas_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    while (next && *next == ',') {
        ...
        switch (flag) {
            ...
            case 'C':
                attributes->memoryManagementPolicy = cas_propertyMemoryManagementPolicyCopy;
                break;

            case '&':
                attributes->memoryManagementPolicy = cas_propertyMemoryManagementPolicyRetain;
                break;
